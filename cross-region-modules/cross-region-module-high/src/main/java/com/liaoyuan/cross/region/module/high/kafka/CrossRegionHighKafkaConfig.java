...
import com.liaoyuan.cross.region.common.kafka.util.CrossRegionKafkaUtils;
...
@Slf4j
public class CrossRegionHighKafkaConfig {
    ...
    public KafkaConsumer<Object,Object> kafkaConsumer(){
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, Constants.CONSUMER_REQUEST_GROUP);
        ...
        KafkaConsumer<Object,Object> kafkaConsumer = new KafkaConsumer<Object, Object>(props);

        if (!discardUnconsumedData){
            return kafkaConsumer;
        }

        CrossRegionKafkaUtils.seekToEnd(kafkaConsumer, Constants.KAFKA_TOPIC_REQUEST);
        log.info("kafka消费组：{}，主题：{} offset的消费位置初始化为最新位置", Constants.CONSUMER_REQUEST_GROUP);
